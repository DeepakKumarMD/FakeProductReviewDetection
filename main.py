...
def view():
    ...
    cursor = mydb.cursor()
    cursor.execute('SELECT c.id,p.product,p.price,p.detail,p.photo,c.rdate FROM cs_cart c,cs_product p where c.pid=p.id and c.bill_id=%s', (bid, ))
    data = cursor.fetchall()
    ...
    if request.method=='POST':
        ...
        if rcode==code:
            ...
            if cnt>0:
                ...
                msg="Your Review has posted , it is genuine"
            else:
                msg="Your Review is fake, you haven't bought this product"
        else:
